...
/**
 * This class handles the User Interface (UI).
 *
 */
...
public class DisplayGUI {
	...
	void showResult() {
		...
		JButton option3 = new JButton("Find day with max calorie intake");
	...
	class Option1 implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			String[] options = { "1. SUNDAY", "2. MONDAY", "3. TUESDAY", "4. WEDNESDAY", 
					"5. THURSDAY", "6. FRIDAY",
					"7. SATURDAY", "0 for current week day" };
			String first = "1. SUNDAY";
			Object selected = JOptionPane.showInputDialog(null, "Please select a date", "Select Date",
	...
	class Option3 implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			displayResult.append(person.getDayConsumedMostCalWeekly().toString() + " :" 
					+ person.getMaxCalories());
			System.out.println(person.getDayConsumedMostCalWeekly().toString() + " :" 
					+ person.getMaxCalories());
		}
	...
	class DisplayOption implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (person != null) {
				System.out.println(
						"Account loaded successfully, please select options: "
								+ "1. input daily calories, "
								+ "2. check past week historic data, "
								+ "3. Find max daily calories in past week, "
								+ "0. exit");
				load.setVisible(false);
				showResult();
			}
			...
			{
				int answer = JOptionPane.showConfirmDialog(null, "We couldn't find your account. "
	...
	class Register implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
				...
				if (each.getText().length() == 0) {
					{
						JOptionPane.showMessageDialog(null, 
								"Please enter your info before proceeding", "Error",
			...
			if (Pattern.matches("[a-zA-Z]+", name))
				...
				JOptionPane.showMessageDialog(null, "Your name cannot be a digit", "Error", 
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			try {
				...
				JOptionPane.showMessageDialog(null, "Height must be in inches", "Error", 
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	void createFoodFrame() {
		...
		for (int i = 0; i < items.size(); i++) {
			button = new JButton(items.get(i).getName()); // create a button with name of item
			button.addActionListener(
					new ButtonListener(items.get(i).getName(), items.get(i).getCalories(), 
							items.get(i).getFat())); // implement action listener
			buttonPanel.add(button); // add each button to panel
			buttonList.add(button); // add button to array list of buttons
		}
		...
		textArea = new JTextArea(10, displayFoodPanel.getWidth()); // Show a list of food items
		JScrollPane sp = new JScrollPane(textArea); // Add a scroll panel to text area
		textArea.setText("You selected the following: \n");
		textArea.setEditable(false); // User should not be able to edit text
		displayFoodPanel.add(sp); // Add scroll panel (with text area) into display panel

		JButton finishFood = new JButton("Proceed to next step");
		finishFood.addActionListener(new CreateExerciseFrame()); // Goes into exercise window
	...
	class CreateExerciseFrame implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			for (int i = 0; i < exItems.size(); i++) {
				// create button with name of item
				eButton = new JButton(exItems.get(i).getExerciseName()); 
				eButton.addActionListener(
						new ExerciseListener(exItems.get(i).getExerciseName(), 
								exItems.get(i).getCaloriesBurnt())); // implement action listener
			...
			// Show a list of exercise items
			exTextArea = new JTextArea(10, displayExercisePanel.getWidth()); 
			JScrollPane sp = new JScrollPane(exTextArea); // Add a scroll panel to text area
			exTextArea.setText("You selected the following: \n");
			exTextArea.setEditable(false); // User should not be able to edit text
			displayExercisePanel.add(sp); // Add scroll panel (with text area) into display panel
			
			JButton finish = new JButton("That's all for today!");
			finish.addActionListener(new CreateResultFrame()); // Goes into result window
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			synchronized (this) {
				person.addDailyCalories(
						new DailyCaloriesInput(WeekDay.getWeekDayType(dateForInput), 
								totalCal, totalCalBurned));
